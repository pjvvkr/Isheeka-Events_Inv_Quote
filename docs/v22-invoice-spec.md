...
12. **Draft auto-refresh on quote re-confirmation (Phase 3).** When a *newer* quote revision is confirmed and the event's invoice is still **draft with zero payments**, the invoice auto-refreshes to match the new confirmed quote. Guardrails: (a) only while draft + unpaid — once Sent or paid, it's the manual "Revise invoice + reason" flow; (b) **refresh in place**, same invoice row + ref number (no new `I-` number); (c) **do not bump `revision_number`** while pre-issue (revisions count from first Send); (d) **always write an auto change-log entry** (old→new total, "synced to quote Q-xxxx", timestamp + user); (e) **preserve invoice-level settings the user already set** (GST toggle/%, invoice discount, due date) and only refresh quote-derived line items + installments, then recompute totals. Implementation note: this changes `createInvoiceFromQuote`'s current dedupe-and-skip into dedupe-and-refresh-if-draft.
13. **Schema ground-truth first.** Before Phase 3 code, author `docs/v22-db-schema.md` from a live `information_schema`/`pg_policies` dump (root-cause fix for the Phase 2 schema mismatch). Phase 3 builds only against that confirmed reference.
14. **DB enum values (from CHECK constraints — verified 13 Jun):** `invoice_payments.payment_mode` ∈ {`cash, neft, upi, cheque`} (lowercase — store these; display friendly labels in UI). `invoices.status` ∈ {draft, sent, partially_paid, paid, overdue, cancelled}. `invoice_installments.status` ∈ {pending, partially_paid, paid, overdue}. `users.role` ∈ {admin, manager, staff} (the role gate). `invoice_payments.installment_id` is **NOT NULL** — every payment must attach to an installment.
